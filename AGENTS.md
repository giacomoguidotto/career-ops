...
| `opportunity-lifecycle.mjs` | Canonical Opportunity contract and guarded commands; passive reads compose Stage, cadence, Attempt, artifact, and candidacy authorities, while explicit Agent-owned work requests and reconciliation are revision-checked under the shared tracker lock |
...
- "Translate the modes to English" → edit all default English files in `modes/`
...
| Wants LinkedIn outreach | `contact` — identifies hiring manager, recruiter, or team peers via web search; drafts a ≤300-char message tailored to the contact type (recruiter / hiring manager / peer / interviewer) |
...
| Asks what to do next for evaluated/applied jobs | `next` |
...
Five canonical triage roles mapped to this repo's label strings. See `docs/agents/triage-labels.md`.
