...
func normalizePipelineTabIndex(index int) int {
	if index >= 0 && index < len(getPipelineTabs()) {
		return index
	}
...
// pipelineTabs keeps the stable lifecycle ordering visible to existing callers
// and tests; rendering calls getPipelineTabs so labels follow the active locale.
var pipelineTabs = getPipelineTabs()
...
type PipelineModel struct {
	...
	discardPendingApp     model.DashboardRow // app awaiting the reason pick
...
func (m PipelineModel) startDiscardFlow(app model.DashboardRow, newStatus string) tea.Msg {
...
func (m PipelineModel) renderHeader() string {
	...
	title := lipgloss.NewStyle().Bold(true).Foreground(m.theme.Blue).Background(m.theme.Surface).Render(i18n.Current.AppTitle)
...
func (m PipelineModel) renderColumnHeader() string {
	...
	segments := []string{
		cell("#", cw.num),
		cell(i18n.Current.ColFit, cw.score),
	}
	if cw.date != 0 {
...
func (m PipelineModel) renderHelp() string {
	...
	segment := func(key, desc string) string {
		return keyStyle.Render(key) + descStyle.Render(" "+strings.TrimSpace(desc))
	}

	groups := []string{
		segment("Enter", "details"),
		segment("o", i18n.Current.HelpOpenURL),
		segment("d", i18n.Current.HelpOpenPDF),
		segment("D", i18n.Current.HelpRegenPDF),
		segment("c", "status"),
		segment("C", i18n.Current.HelpColumns),
		segment("p", i18n.Current.HelpProgress),
		segment("jk", i18n.Current.HelpNav),
		segment("hl", i18n.Current.HelpTabs),
		segment("^D/^U", "half"),
		segment("Space/b", i18n.Current.HelpPage),
		segment("g/G", i18n.Current.HelpTopEnd),
		segment("/", i18n.Current.HelpSearch),
		segment("s", i18n.Current.HelpSort),
		segment("r", i18n.Current.HelpRefresh),
		segment("v", i18n.Current.HelpView),
		segment("t", i18n.Current.HelpLanguage),
		segment("q", i18n.Current.HelpQuit),
	}
