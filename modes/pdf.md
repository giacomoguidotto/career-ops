...
2. **Contact = email + LinkedIn + portfolio. No phone, photo, or full address.**
...
20. Report: PDF path, number of pages (must be 1), keyword coverage %, and any skill gaps from Step 4 still unaddressed. If the page count came back over budget, trim and regenerate before handing the PDF to the user.
...
| `candidate.phone` | string | Omit or leave empty. The fork's generated contact rows intentionally exclude phone numbers. |
...
| `candidate.location` | string | Optional short "City, Country" from `profile.yml`; never include a street-level address. |
...
| `competencies` | string[] | About 6 JD-matched keyword phrases, each proven in the experience or projects below. |
| `experience[]` | object | `company`, `role`, `location` (optional), `dates`, `bullets` (reordered, keyword-injected); fold technologies into bullets or a per-role `Tech:` line. |
...
| `skills[]` | object | Optional and off by default. When the market or role expects a standalone Skills section, use `category` + `items` (comma-separated string or string array). |
