...
2. **For each surviving pending URL**:
   ...
   d. **Execute full auto-pipeline**: Evaluation A-F → Report .md → PDF (if score >= `auto_pdf_score_threshold`) → Tracker. Read `modes/_custom.md` -> `Evaluation And Advancement Policy` plus any applicable pipeline/evaluation rules, if they exist, and apply those overrides to every item. Default (if absent or silent): standard pipeline execution.
