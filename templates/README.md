...
Defines the canonical application lifecycle state machine — the 12 states (`Evaluated`, `Application Ready`, `Applied`, `Outreach Ready`, `Responded`, `Interview Ready`, `Offer`, `Offer Ready`, `Accepted`, `Rejected`, `Discarded`, `SKIP`) with each state's owner, suggested next step, and aliases for common variants. All pipeline scripts and the dashboard validate statuses against this file.
