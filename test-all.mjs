...
`, 'utf-8');
  ...
  const validPluginProviderResult = run(NODE, ['validate-portals.mjs', '--file', validProviderPluginPath]);
  if (validPluginProviderResult !== null && validPluginProviderResult.includes('0 errors')) {
    pass('validate-portals accepts installed provider-plugin ids');
  } else {
    fail('validate-portals should accept installed provider-plugin ids');
  }
