...
const PROFILE_LOCK_PATH = 'config/.career-profile.lock';
const PROFILE_LOCK_TIMEOUT_MS = 60_000;
...
async function acquireProfileLock(root) {
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        const timeout = new Error(`Timed out waiting for Career profile lock at ${path}`);
...
export async function reconcileCareerProfile(request, { root = process.cwd() } = {}) {
  const lock = await acquireProfileLock(root);
  try {
