...
import {
  advanceOpportunity,
  checkOpportunityPursuit,
  reviewWaitingOpportunities,
  selectRelatedOpportunities,
} from './career-opportunity-pursuit.mjs';
...
register(capabilityDescriptor(
  ...
  'Select the exclusive Career-native set of eligible related Opportunities without writing.',
  (input) => selectRelatedOpportunities(input),
  () => checkOpportunityPursuit(),
));

register(capabilityDescriptor(
  'career.opportunity.advance/v1',
  'Reserve or reconcile revision-checked Agent-owned planning while preserving real-world evidence gates.',
  (input) => advanceOpportunity(input),
  () => checkOpportunityPursuit(),
));

register(capabilityDescriptor(
  'career.opportunity.review-waiting/v1',
  'Review externally owned waits from confirmed native Attempt and cadence evidence without changing factual Stage.',
  (input) => reviewWaitingOpportunities(input),
  () => checkOpportunityPursuit(),
));
